...
<%@page import="static web.util.StringUtils.normalizeNull" %>
...
<form name="input" action="registrazione.do" method="post">
	<div>
		...
		<% } %><br /><br />
		<span>Username: <input type="text" name="username" value="<%= normalizeNull(username) %>" size="20" /></span>
		<% if (errori.get("username")!=null) { %>
			<span class="errore"><%= errori.get("username") %></span>
		<% } %>
		<br />
		<span>Password: <input type="password" name="password" value="<%= normalizeNull(password) %>" size="20" /></span>
